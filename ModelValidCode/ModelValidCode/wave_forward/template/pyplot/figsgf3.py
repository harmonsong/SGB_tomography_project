...
highcut = 1.0 / bands[:,0]
fs = 1.0/(t[1]-t[0])
wave0 = np.zeros(Dz.shape) #3-7s
wave1 = np.zeros(Dz.shape) #5-10s
wave2 = np.zeros(Dz.shape) #8-15s
wave3 = np.zeros(Dz.shape) #12-20s
order = 2

for i in range(Dz.shape[0]):
    wave0[i,:] = butter_bandpass_filter(Dz[i,:], lowcut[0], highcut[0], fs, order)
    wave1[i,:] = butter_bandpass_filter(Dz[i,:], lowcut[1], highcut[1], fs, order)
    wave2[i,:] = butter_bandpass_filter(Dz[i,:], lowcut[2], highcut[2], fs, order)
    wave3[i,:] = butter_bandpass_filter(Dz[i,:], lowcut[3], highcut[3], fs, order)
#==============================================================================
...
fig = plt.figure(figsize = (8, 6), dpi = 80)
for i in range(0,nsta,stride):
  plt.plot(tc, Dz[i]/np.max(abs(Dz[i]))*10+r[i]/1e3, 'b', antialiased='False', linewidth=0.4)
plt.ylim([0, 800])
...
fig = plt.figure(figsize = (8, 6), dpi = 80)
for i in range(0,nsta,stride):
  plt.plot(tc, wave0[i]/np.max(abs(wave0[i]))*10+r[i]/1e3, 'b', antialiased='False', linewidth=0.4)
plt.ylim([0, 800])
plt.title('Seismogram for 3-7s')
plt.ylabel('distance (km)')
plt.xlabel('time (s)')

fig = plt.figure(figsize = (8, 6), dpi = 80)
for i in range(0,nsta,stride):
  plt.plot(tc, wave1[i]/np.max(abs(wave1[i]))*10+r[i]/1e3, 'b', antialiased='False', linewidth=0.4)
plt.ylim([0, 800])
plt.title('Seismogram for 5-10s')
plt.ylabel('distance (km)')
plt.xlabel('time (s)')

fig = plt.figure(figsize = (8, 6), dpi = 80)
for i in range(0,nsta,stride):
  plt.plot(tc, wave2[i]/np.max(abs(wave2[i]))*10+r[i]/1e3, 'b', antialiased='False', linewidth=0.4)
plt.ylim([0, 800])
plt.title('Seismogram for 8-15s')
plt.ylabel('distance (km)')
plt.xlabel('time (s)')

fig = plt.figure(figsize = (8, 6), dpi = 80)
for i in range(0,nsta,stride):
  plt.plot(tc, wave3[i]/np.max(abs(wave3[i]))*10+r[i]/1e3, 'b', antialiased='False', linewidth=0.4)
plt.ylim([0, 800])
plt.title('Seismogram for 12-20s')
plt.ylabel('distance (km)')
plt.xlabel('time (s)')
