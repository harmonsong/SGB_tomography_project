...
def check_status(jobs_status,jobs_ids):
    ...
    for flag_run in flag_runs:
        ...
        if state == 'DONE':
            ...
            email_to = [user_recive]
            email_title = 'TaiYi Job'
            email_content = 'Job finished fail! Job ID: '+job_id + ' source: '+ str(flag_run+1)
            yag_server.send(email_to, email_title, email_content)
            print("Job finished fail! Job ID: ", job_id, ' source: ', str(flag_run+1))
            ...
            yag_server.send(email_to, email_title, email_content)
            print("Job start running! Job ID: ", job_id, ' source: ', str(flag_run+1))
